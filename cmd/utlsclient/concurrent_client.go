...
func ConcurrentTestMain() {
	...
	flag.Parse()
	ConcurrentTestMainWithFlags(*proxyAddr, *token, *targetURL, *concurrency, *totalRequests, *timeout)
}

// ConcurrentTestMainWithFlags 使用已解析的flag值运行并发测试
func ConcurrentTestMainWithFlags(proxyAddr, token, targetURL string, concurrency, totalRequests int, timeout time.Duration) {
