...
- `ClearRedisAfterPersist`：为true时，后台持久化完成后清理对应键，避免冗余占用；为false时保留缓存以提升读取命中率。
- `PersistBatchSize` 与 `PersistInterval`：批次阈值优先触发，定时器兜底触发，二者共同决定后台工作线程的节奏与吞吐。
- 优雅关闭：`TileStorage` 在`Close()`阶段刷新队列并执行最终持久化，随后依据`ClearRedisAfterPersist`策略清理Redis键。
