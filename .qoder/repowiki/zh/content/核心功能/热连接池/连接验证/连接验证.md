...
- `ValidateConnection`：基础健康检查入口
- `ValidateConnectionWithPath`：HEAD请求路径验证，适合轻量可达性测试
- `ValidateConnectionWithFullURL`：完整URL验证，支持定制请求构建
- `ValidateConnectionWithGET`：严格GET验证，带 `maxBodySize` 限制
- `QuickHealthCheck`：快速健康探测，仅检查基本可用性
- `BatchValidateConnections`：批量执行路径验证，返回成功/失败计数

**章节来源**
- [connection_validator.go](file://utlsclient/connection_validator.go#L15-L19)
...
- [connection_validator.go](file://utlsclient/connection_validator.go#L241-L261)
- [connection_validator.go](file://utlsclient/connection_validator.go#L225-L238)

### 快速健康检查与协作

- `QuickHealthCheck` 在连接空闲或轻度超时场景下用于快速确认连接是否可用
- `HealthChecker` 在需要时会调用验证器进行二次确认
...
- `TestTimeout`：控制验证请求的超时，默认 10 秒，对应 TOML `test_timeout`
- `MaxRetries`：用于连接创建阶段的重试上限（验证阶段通常不做多次重试）

**节来源**
- [utlshotconnpool.go](file://utlsclient/utlshotconnpool.go#L186-L202)
- [config/config.toml](file://config/config.toml#L18-L22)
