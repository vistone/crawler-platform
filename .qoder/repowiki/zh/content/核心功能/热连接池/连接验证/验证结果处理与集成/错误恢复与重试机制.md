...
系统实现了基于有界循环的重试机制，通过配置的最大重试次数（默认为1次）来控制重试行为。在每次重试中依次执行IP获取、访问控制验证、TLS指纹选择和连接创建，仅在步骤失败时进入下一次迭代。该改进避免了在所有IP被拒绝时可能导致的无限递归和栈溢出风险，提高了系统的稳定性和安全性。
...
UTLSClient->>Pool : 获取IP
Pool-->>UTLSClient : 返回IP
UTLSClient->>Pool : 验证IP访问权限
Pool-->>UTLSClient : 验证结果
alt 验证失败
UTLSClient->>UTLSClient : 增加重试计数
UTLSClient->>UTLSClient : 计算退避延迟
UTLSClient->>UTLSClient : 等待延迟
else 验证成功
UTLSClient->>Pool : 选择TLS指纹
Pool-->>UTLSClient : 返回指纹
UTLSClient->>Pool : 创建并验证连接
Pool-->>UTLSClient : 连接或错误
alt 创建成功
...
**章节来源**
- [utlshotconnpool.go](file://utlsclient/utlshotconnpool.go#L534-L571) - *更新了重试机制，从递归改为有界循环*
- [utlsclient.go](file://utlsclient/utlsclient.go#L104-L118) - *更新了重试逻辑*
...
**章节来源**
- [utlshotconnpool.go](file://utlsclient/utlshotconnpool.go#L534-L571) - *更新了错误恢复流程，采用有界循环重试*
- [utlsclient.go](file://utlsclient/utlsclient.go#L80-118) - *更新了重试逻辑*

### 恢复策略矩阵

| 错误类型 | 检查策略 | 恢复策略 | 重试次数 | 退避策略 |
|----------|----------|----------|----------|----------|
| 连接超时 | QuickHealthCheck | 重新获取连接 | 1次 | 线性增长 |
| 连接断开 | 健康检查 | 创建新连接 | 1次 | 固定延迟 |
| 403禁止 | 访问控制 | 加入黑名单 | 0次 | 不重试 |
| 5xx服务器错误 | 健康检查 | 重新尝试 | 1次 | 指数退避 |
