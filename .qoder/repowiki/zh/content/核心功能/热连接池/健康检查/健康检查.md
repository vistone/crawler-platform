...
健康检查循环由连接池的后台维护任务驱动，使用 `done` 通道实现优雅停止，并通过 `sync.WaitGroup` 等待所有任务收敛。后台任务包括 healthCheckLoop、cleanupLoop、blacklistCheckLoop、dnsUpdateLoop。
...
### 配置项 HealthCheckInterval

健康检查触发频率由 `PoolConfig.HealthCheckInterval` 控制，对应 TOML 中的 `health_check_interval`（秒）。建议在 15–60 之间按网络情况调整。
...
- `CheckAllConnections()`：复制当前连接快照并逐个执行检查
- `CleanupUnhealthyConnections()`：批量移除标记为不健康的连接
